...
const useGameClient = () => {
  ...
  const hasKey = useCallback((keyId?: string) => {
    if (!player || !keyId) return true;
    return player.inventory.some(item => item.id === keyId && item.type === 'key');
  }, [player]);
  
  // Initialize game
  const initializeGame = useCallback((playerData: Player, startingRoomId: string, difficulty: Difficulty) => {
  ...
  // Player actions
  const movePlayer = useCallback((newPosition: Position, targetRoomId: string) => {
    ...
    if (!validExit || (validExit.isLocked && !hasKey(validExit.requiredKey))) {
      addNotification({
        id: `move-${Date.now()}`,
        type: 'warning',
  ...
  const useItem = useCallback((itemId: string, targetId?: string, targetType?: string) => {
  ...
  const pickupItem = useCallback((item: InventoryItem) => {
    if (!player || !currentRoom) return false;
  ...
  // Combat system
  const attackEnemy = useCallback((enemyId: string, weaponId?: string) => {
    if (!player) return false;
    ...
    const weapon = weaponId ? 
      player.inventory.find(item => item.id === weaponId) : null;
  ...
  // Save/Load system
  const saveGame = useCallback((slotNumber: number, saveName?: string) => {
    if (!player || !currentRoom) return false;
    ...
    localStorage.setItem(`haunted-asylum-save-${slotNumber}`, JSON.stringify(saveData));
    ...
    return true;
  }, [player, currentRoom, gameState, rooms, gameEvents, settings.difficulty, addNotification]);
  ...
  const updateSettings = useCallback((newSettings: Partial<GameSettings>) => {
    ...
    localStorage.setItem('haunted-asylum-settings', JSON.stringify({
      ...settings,
      ...newSettings
    }));
  }, [settings]);
      ...
      if (deltaTime > 0 && deltaTime < 1000) {
        ...
        setCurrentRoom(prevRoom => {
          if (prevRoom) {
            ...
            return {
              ...prevRoom,
              hauntingLevel: Math.min(100, prevRoom.hauntingLevel + hauntingIncrease)
            };
          }
          return prevRoom;
        });
        ...
        setPlayer(prevPlayer => {
          if (prevPlayer && currentRoom && currentRoom.hauntingLevel > 50) {
            const sanityLoss = (currentRoom.hauntingLevel / 100) * (deltaTime / 1000) * 2;
            return {
              ...prevPlayer,
