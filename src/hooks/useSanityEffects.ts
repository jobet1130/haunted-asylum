...
interface HallucinationEffect {
  id: string;
  type: 'flicker' | 'distortion' | 'shadow' | 'text-scramble' | 'false-enemy' | 'phantom-sound';
  intensity: number;
  duration: number;
  timestamp: number;
...
const useSanityEffects = (player: Player | null): UseSanityEffectsReturn => {
  ...
  const [activeVisualEffects, setActiveVisualEffects] = useState<VisualEffect[]>([]);
  ...
  const getSanityEffectConfig = useCallback((sanity: number): SanityEffectConfig => {
  ...
  const triggerHallucination = useCallback((type: HallucinationEffect['type']) => {
    if (!player) return;
    ...
    setTimeout(() => {
      ...
      if (visualEffect) {
        setActiveVisualEffects(prev => prev.filter(effect => effect.id !== visualEffect.id));
      }
    }, config.duration);
  ...
  useEffect(() => {
    ...
    const scheduleNextEffect = () => {
      ...
      const effectTypes: HallucinationEffect['type'][] = ['flicker', 'distortion'];
